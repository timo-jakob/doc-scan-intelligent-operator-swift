...
// MARK: - Default Config Path Tests

extension ConfigurationTests {
    func testDefaultConfigPathResolvesToDotscanDir() {
    ...
    func testDefaultConfigDirResolvesToDotscanDir() {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        XCTAssertEqual(Configuration.defaultConfigDir, "\(home)/.docscan")
    }
    ...
    func testSaveCreatesIntermediateDirectories() throws {
        ...
        let config = Configuration.defaultConfiguration
        try config.save(to: nested)

        XCTAssertTrue(FileManager.default.fileExists(atPath: nested))
    }

    func testSaveAndLoadRoundTripNestedPath() throws {
        let nested = tempDirectory
        ...
        XCTAssertEqual(loaded.modelName, "nested-vlm")
        XCTAssertEqual(loaded.textModelName, "nested-text")
        XCTAssertEqual(loaded.maxTokens, 128)
    }
}
